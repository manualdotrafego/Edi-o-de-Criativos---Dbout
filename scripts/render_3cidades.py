...
cities = [
    ("QUEIMADAS",  "queimadas", "Queimadas"),
    ("FAZENDA",    "fazenda",   "Fazenda"),
    ("JANUÁRIA",   "januaria",  "Januária"),
]
...
for city_label, slug, city_name in cities:
    text = f"Em {city_name}, agora é diferente! Você ganha o aparelho, documentação, raio-X e um super clareamento. Clique no botão abaixo e garanta a sua vaga!"
    r = requests.post(f"https://api.elevenlabs.io/v1/text-to-speech/{VOICE_ID}",
        headers={"xi-api-key": API_KEY, "Content-Type": "application/json"},
        json={"text": text, "model_id": "eleven_multilingual_v2",
              "voice_settings": {"stability": 0.4, "similarity_boost": 0.8, "style": 0.3, "use_speaker_boost": True}})
    open(f"/tmp/{slug}.mp3", "wb").write(r.content)
    print(f"  ✅ {city_name}")
...
for city_label, slug, city_name in cities:
    print(f"\n🏙️  {city_label}")
    audio     = f"/tmp/{slug}.mp3"
    narration = f"Em {city_name}, agora é diferente! Você ganha o aparelho, documentação, raio-X e um super clareamento. Clique no botão abaixo e garanta a sua vaga!"
...
zip_path = f"{DL}/queimadas_fazenda_januaria.zip"
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for mp4 in outputs:
        zf.write(mp4, os.path.basename(mp4))
print(f"\n📦 {zip_path} ({os.path.getsize(zip_path)/1024/1024:.1f} MB) — {len(outputs)} vídeos")
